...
describe('numberToText', () => {
  TEST_SET.forEach(([input, nlExpectedOutput, enExpectedOutput]) => {
    test(`[NL] if it converts ${input} correctly`, () => {
      const result = nlConverter.numberToText(input as number);
      expect(result).toBe(nlExpectedOutput);
    });

    test(`[EN] if it converts ${input} correctly`, () => {
      const result = enConverter.numberToText(input as number);
...
describe('textToNumber', () => {
  TEST_SET.forEach(([expectedOutput, nlInput, enInput]) => {
    test(`[NL] if it converts ${nlInput} correctly`, () => {
      const result = nlConverter.textToNumber(nlInput as string);
      expect(result).toBe(expectedOutput);
    });

    test(`[EN] if it converts ${enInput} correctly`, () => {
      const result = enConverter.textToNumber(enInput as string);
      expect(result).toBe(expectedOutput);
    });
  });
});
